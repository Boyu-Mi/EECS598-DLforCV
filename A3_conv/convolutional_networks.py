...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    pad = conv_param['pad']
    padding_tuple = (pad, pad, pad, pad)  # last 2 dimensions, pad on each side
    x_pad = torch.nn.functional.pad(x, padding_tuple, "constant", 0)  # shape (N, C, H + 2 * pad, W + 2 * pad)
    h, w_shape, hh, ww = x.shape[2], x.shape[3], w.shape[2], w.shape[3]
    N, F = x.shape[0], w.shape[0]
    h_ = 1 + (h + 2 * pad - hh) // stride  # h'
    w_ = 1 + (w_shape + 2 * pad - ww) // stride  # w'
    out = torch.empty(x.shape[0], w.shape[0], h_, w_).to(device = x.device, dtype = x.dtype)
    for i in range(h_):
        for j in range(w_):
            i_x = i * stride
            j_x = j * stride
            slice_ = x_pad[:, :, i_x: i_x + hh, j_x: j_x + ww]  # shape (N, C, hh, ww)
            slice_ = slice_.reshape(N, -1)  # shape (N, C * hh * ww)
            kernel_slice = w.reshape(F, -1)  # shape(F, C * hh * ww)
            unit = slice_ @ kernel_slice.t() + b.reshape(1, -1)  # (N ,F) + (1, F)
            out[:, :, i, j] = unit
  ...
  def backward(dout, cache):
      ...
      for i in range(h_):
          for j in range(w_):
              ...
              j_x = j * stride
              # (N, F) @ (F, D)
              dout_slice = dout[:, :, i, j]  # (N, F)
              w_sclice = w.view(F, -1)  # (F, D) 
              dslice = dout_slice @ w_sclice
              # to produce a tensor of shape (N, C, hh, ww)
              dx_pad[:, :, i_x: i_x + hh, j_x: j_x + ww] += dslice.view(N, C, hh, ww) # shape (N, C, hh, ww)
              x_slice = x_pad[:, :, i_x: i_x + hh, j_x: j_x + ww]
              tmp = dout_slice.t().mm(x_slice.reshape(N, -1)) #(F, N) @ (N, D)
              dw += tmp.reshape(F, C, hh, ww)
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    N, C, H, W = x.shape
    h_ = 1 + (H - pool_height) // stride  # h'
    w_ = 1 + (W - pool_width) // stride  # w'
    out = torch.empty(N, C, h_, w_).to(device = x.device, dtype = x.dtype)
    for i in range(h_):
        for j in range(w_):
            i_x = i * stride
            j_x = j * stride
            slice_ = x[:, :, i_x: i_x + pool_height, j_x: j_x + pool_width]  # shape (N, C, hh, ww)
            val, _ = torch.max(slice_, 2)
            out[:, :, i, j], _ = torch.max(val, 2)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  #   dx = torch.empty(x.shape).to(device = x.device, dtype = x.dtype)
  #   for i in range(h_):
  #       for j in range(w_):
  #           i_x = i * stride
  #           j_x = j * stride
  #           slice_ = x[:, :, i_x: i_x + pool_height, j_x: j_x + pool_width]  # shape (N, C, hh, ww)
  #           dout_slice = dout[:, :, h_, w_]
  #           for ii in range(N):
  #             for jj in range(C):
  #               for kk in range(i_x, i_x + pool_height):
  #                 for ll in range(j_x, j_x + pool_width):
  #                   if dout_slice[ii, jj] == x[ii, jj, kk, ll]:
  #                     dx[ii, jj, kk, ll] = dout_slice[ii, jj]
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    ############################################################################
    # Replace "pass" statement with your code
    zeroConv = torch.zeros(num_filters)
    C, H, W = input_dims
    self.params["W1"] = torch.normal(0, weight_scale, size=(num_filters, C, filter_size, filter_size)).to(dtype).to(device)
    self.params["b1"] = torch.zeros_like(zeroConv).to(dtype).to(device)

    # the size after Conv_ReLU : (N, F, H, W)
    # after maxpooling: (N, F, H / 2, W / 2)
    zeroH1 = torch.zeros(hidden_dim)
    self.params["W2"] = torch.normal(0, weight_scale, size=(num_filters * H // 2 * W // 2, hidden_dim)).to(dtype).to(device)
    self.params["b2"] = torch.zeros_like(zeroH1).to(dtype).to(device)

    zeroO = torch.zeros(num_classes)
    self.params["W3"] = torch.normal(0, weight_scale, size=(hidden_dim, num_classes)).to(dtype).to(device)
    self.params["b3"] = torch.zeros_like(zeroO).to(dtype).to(device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    # print(conv_out.shape) #(N, F, H / 2, W / 2)
    hidden, hidden_cache = Linear_ReLU.forward(conv_out, W2, b2)
    scores, out_cache = Linear.forward(hidden, W3, b3)
    ...
    dhidden, dw3, db3 = Linear.backward(dscore, out_cache)
    dconv_out, dw2, db2 = Linear_ReLU.backward(dhidden, hidden_cache)
    dX, dw1, db1 = Conv_ReLU_Pool.backward(dconv_out, conv_cache)

    grads['W1'] = dw1.to(W1.device) + 2 * self.reg * W1
    grads['W2'] = dw2.to(W2.device) + 2 * self.reg * W2
    grads['W3'] = dw3.to(W3.device) + 2 * self.reg * W3
    grads['b1'] = db1
    grads['b2'] = db2
    grads['b3'] = db3
    ############################################################################
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      running_mean = momentum * running_mean + (1 - momentum) * sample_mean
      running_var = momentum * running_var + (1 - momentum) * sample_var  
      cache = (x, sample_mean, sample_var, x_hat, out, gamma, beta, bn_param)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
      ...
      out = gamma * x_hat + beta
      cache = (x, running_mean, running_var, x_hat, gamma, beta, bn_param)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    else:
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    x, sample_mean, sample_var, x_hat, out, gamma, beta, bn_param = cache
    mode = bn_param["mode"]
    eps = bn_param.get('eps', 1e-5)
    momentum = bn_param.get('momentum', 0.9)
    ...
    if mode == "train":
      dLdx_hat = dout * gamma #(N, D) * (1, D) = (N, D)
      dLdsample_var = torch.sum(dLdx_hat * (x - sample_mean), dim = 0) * (-1 / 2) * ((sample_var + eps) ** (-3 / 2))

      term1 = -1 * torch.sum(dLdx_hat, dim = 0) / torch.sqrt(sample_var + eps)
      term2 = dLdsample_var * (-2 / N) * torch.sum(x - sample_mean, dim = 0)
      dLdmean = term1 + term2

      term1 = dLdx_hat / torch.sqrt(sample_var + eps)
      term2 = dLdsample_var * (2 / N) * (x - sample_mean)
      term3 = dLdmean / N
      dx = term1 + term2 + term3
      # (https://arxiv.org/abs/1502.03167)   
    elif mode == "test":
      x, running_mean, running_var, x_hat, gamma, beta, bn_param = cache
      dLdx_hat = dout * gamma
      dx = dLdx_hat / torch.sqrt(running_var + eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = x.shape
    x_ = x.transpose(0, 1).reshape(C, N * H * W).t()
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = dout.shape
    dout_= dout.transpose(0, 1).reshape(C, N * H * W).t() 
    dx, dgamma, dbeta = BatchNorm.backward_alt(dout_, cache)
