# some code is to be implemented.....
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
            ...
            Convlayer = nn.Conv2d(in_channel, out_channel, (3, 3), stride=(1, 1), padding=(1, 1))
            mean = 0
            std = 0.01
            nn.init.normal_(Convlayer.weight, mean, std)
            nn.init.constant_(Convlayer.bias, 0)
            stem_cls.append(Convlayer)
            stem_cls.append(nn.ReLU())

            Convlayer_ = nn.Conv2d(in_channel, out_channel, (3, 3), stride=(1, 1), padding=(1, 1))
            nn.init.normal_(Convlayer_.weight, mean, std)
            nn.init.constant_(Convlayer_.bias, 0)
            stem_box.append(Convlayer_)
        ...
        self.pred_cls = nn.Conv2d(pred_in_channels, num_classes, (3, 3), stride=(1, 1), padding=(1, 1))
        self.pred_box = nn.Conv2d(pred_in_channels, 4, ker_size, stride=(1, 1), padding=(1, 1))
        self.pred_ctr = nn.Conv2d(pred_in_channels, 1, ker_size, stride=(1, 1), padding=(1, 1))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        key_lst = ["p3", "p4", "p5"]
        for k in key_lst:
            cls_pred = self.pred_cls(self.stem_cls(feats_per_fpn_level[k]))
            B, num_classes, H, W = cls_pred.shape
            class_logits[k] = cls_pred.reshape(B, H * W, num_classes)

            box_pred = self.pred_box(self.stem_box(feats_per_fpn_level[k]))
            B, _, H, W = box_pred.shape
            boxreg_deltas[k] = box_pred.reshape(B, H * W, 4)

            ctr_pred = self.pred_ctr(self.stem_box(feats_per_fpn_level[k])) # share the same stem
            centerness_logits[k] = ctr_pred.reshape(B, H * W, 1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        fpn_feats_shapes = {
            level_name: feat.shape for level_name, feat in fpn_feat.items()
        }
        ...
        for i in range(images.shape[0]):
            ...
            for level in FPN_LEVELS:
                _deltas[level] = fcos_get_deltas_from_locations(locations_per_fpn_level[level],
        ...
        #  1. gt boxes(B, N, 5) -> one hot code of class
        ...
        #  2. loss_box pred_boxreg_deltas to gt_boxes
        loss_box = 0.25 * F.l1_loss(
            pred_boxreg_deltas, matched_gt_deltas, reduction="none"
        )

        loss_box[matched_gt_deltas < 0] *= 0.0
        b, n, _ = pred_cls_logits.shape
        gt_centerness = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4)).reshape(b, n, -1)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            #      than provided threshold in arguments.
            idx = scores >= test_score_thresh
            level_pred_scores = scores[idx]
            ...
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas[idx], level_locations[idx], stride)
