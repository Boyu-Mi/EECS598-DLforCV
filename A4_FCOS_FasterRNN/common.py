...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params = nn.ModuleDict()
        channel_nums = [feature_shape[1] for _, feature_shape in dummy_out_shapes] # now it's [64, 160, 400]
        for i in range(len(channel_nums)):
            name_c_to_p = "CP" + str(i + 3)
            name_p = "p" + str(i + 3)
            self.fpn_params[name_c_to_p] = nn.Conv2d(channel_nums[i], out_channels, (1, 1))
            self.fpn_params[name_p] = nn.Conv2d(out_channels, out_channels, (3, 3), stride=(1, 1), padding=(1, 1))
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        fpn_feats["p5"] = self.fpn_params["CP5"](backbone_feats["c5"])
        p5_upsample = F.interpolate(fpn_feats["p5"], scale_factor=2)
        fpn_feats["p5"] = self.fpn_params["p5"](fpn_feats["p5"])

        fpn_feats["p4"] = self.fpn_params["CP4"](backbone_feats["c4"]) + p5_upsample
        p4_upsample = F.interpolate(fpn_feats["p4"], scale_factor=2)
        fpn_feats["p4"] = self.fpn_params["p4"](fpn_feats["p4"])

        fpn_feats["p3"] = self.fpn_params["CP3"](backbone_feats["c3"]) + p4_upsample
        fpn_feats["p3"] = self.fpn_params["p3"](fpn_feats["p3"])
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
def get_fpn_location_coords(
    ...
    # print(shape_per_fpn_level.items())
    for level_name, feat_shape in shape_per_fpn_level.items():
        level_stride = strides_per_fpn_level[level_name]
        ...
        ######################################################################
        _, _, H, W = feat_shape
        first_col = torch.arange(H).reshape(-1, 1).repeat(1, W).reshape(-1, 1)
        sec_col = torch.arange(W).repeat(H).reshape(-1, 1)
        tmp = torch.hstack((first_col, sec_col))
        location_coords[level_name] = ((tmp + 0.5) * level_stride).to(device).to(dtype)
        # B, C, H, W = feat_shape
        # H_vec = torch.arange(H, dtype=dtype, device=device).reshape(1,-1).repeat(W,1).permute(1,0).reshape(-1,1)
        # W_vec = torch.arange(W, dtype=dtype, device=device).reshape(1,-1).repeat(1,H).reshape(-1,1)
        # # HW = torch.hstack((H_vec, W_vec))
