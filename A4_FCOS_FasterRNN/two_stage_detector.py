...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
            if i == 0:
                ...
                in_channel = stem_channels[i - 1]
            out_channel = stem_channels[i]
            conv = nn.Conv2d(in_channel, out_channel, (3, 3), stride=(1, 1), padding=(1, 1))
            mean = 0
            std = 0.01
        ...
        # predict for objectness:
        pred_in_channels = stem_channels[-1]
        self.pred_obj = nn.Conv2d(pred_in_channels, num_anchors, (1, 1), stride=(1, 1))
        self.pred_box = nn.Conv2d(pred_in_channels, 4 * num_anchors, (1, 1), stride=(1, 1))
        mean = 0
        std = 0.01
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        object_logits = {}
        boxreg_deltas = {}
        batch_size, _, h, w = feats_per_fpn_level["p3"].shape

        levels = ["p3", "p4", "p5"]
        for key_ in levels:
            obj_logits = self.pred_obj(self.stem_rpn(feats_per_fpn_level[key_]))
            box_logits = self.pred_box(self.stem_rpn(feats_per_fpn_level[key_]))
            # obj_logits should be tensor of shape (batch_size, num_anchors, H, W)
            assert self.num_anchors == obj_logits.shape[1]
            assert 4 * self.num_anchors == box_logits.shape[1]
            object_logits[key_] = obj_logits.reshape(batch_size, -1)
            boxreg_deltas[key_] = box_logits.reshape(batch_size, 4, -1).permute(0, 2, 1)
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            ##################################################################
            # TODO: generate a tensor of shape (H * W, 4)
            # the center of box is location (H * W, 2)
            # area = `(stride_scale * FPN level stride) ** 2`
            # new_width = sqrt(area / aspect ratio)
            # new_height = area / new_width
            ctr_x = locations[:, 0]
            ctr_y = locations[:, 1]
            ...
            new_height = area / new_width
            top = (ctr_y + new_height / 2).view(-1, 1)
            bot = (ctr_y - new_height / 2).view(-1, 1)
            right = (ctr_x + new_width / 2).view(-1, 1)
            left = (ctr_x - new_width / 2).view(-1, 1)
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    ##########################################################################
    # h = max(0, min(x21, x22) - max(x11, x12))
    # w = max(0, min(y11, y12) - max(y21, y22))
    # intersection = h * w
    # iou = intersection / area1 + area2 - intersection
    m = boxes1.shape[0]
    n = boxes2.shape[0]
    # form : left, bot, right, top
    x11, y11, x21, y21 = tuple(
        map(lambda i: boxes1[:, i].view(-1, 1).repeat(1, n), (i for i in range(4)))
    )
    x12, y12, x22, y22 = tuple(
        map(lambda i: boxes2[:, i].view(-1, 1).repeat(1, m).t(), (i for i in range(4)))
    )
    # following 2 lines can help to debug
    # assert x11.shape[0] == m and y11.shape[1] == n
    # assert x12.shape[0] == m and y12.shape[1] == n
    h = torch.min(x21, x22) - torch.max(x11, x12)
    w = torch.min(y21, y22) - torch.max(y11, y12)
    h[h < 0] = 0
    w[w < 0] = 0
    intersection = h * w #  intersection[i, j] is the area of intersection between the ith location of box1 and the jth location of box2
    area1 = (boxes1[:, 2] - boxes1[:, 0]) * (boxes1[:, 3] - boxes1[:, 1])
    area2 = (boxes2[:, 2] - boxes2[:, 0]) * (boxes2[:, 3] - boxes2[:, 1])
    area = area1.view(-1, 1).repeat(1, n) + area2.view(-1, 1).repeat(1, m).t()  # area[i, j] is the sum of the ith location of box1 and the jth location of box2
...
def rcnn_get_deltas_from_anchors(
    ...
    if gt_boxes.shape[1] == 5:
        x1, y1, x2, y2, c = tuple(
            map(lambda i: gt_boxes[:, i].view(-1, 1), (i for i in range(5)))
        )
    else:
        x1, y1, x2, y2 = tuple(
            map(lambda i: gt_boxes[:, i].view(-1, 1), (i for i in range(4)))
        )
    x1a, y1a, x2a, y2a = tuple(
        map(lambda i: anchors[:, i].view(-1, 1), (i for i in range(4)))
    )
    # form : left, bot, right, top
    #        x1, y1, x2, y2
    bh = y2 - y1
    ...
    deltas = torch.hstack((dx, dy, dw, dh))
    # print(gt_boxes[:, :4])
    background_mask = (gt_boxes[:, :4] == torch.tensor([-1, -1, -1, -1], device=gt_boxes.device))[:, 0]
    # print(background_mask)
    deltas[background_mask, :] = torch.tensor([-1e8, -1e8, -1e8, -1e8], device=gt_boxes.device)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rcnn_apply_deltas_to_anchors(
    ...
    dx, dy, dw, dh = tuple(
        map(lambda i: deltas[:, i].view(-1, 1), (i for i in range(4)))
    )
    x1a, y1a, x2a, y2a = tuple(
        map(lambda i: anchors[:, i].view(-1, 1), (i for i in range(4)))
    )
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        location_per_fpn_level = get_fpn_location_coords(fpn_feats_shapes, strides_per_fpn_level,
                                                         feats_per_fpn_level["p3"].dtype,
                                                         feats_per_fpn_level["p3"].device)
        anchors_per_fpn_level = generate_fpn_anchors(location_per_fpn_level, strides_per_fpn_level,
        ...
        for i in range(gt_boxes.shape[0]):
            matched_gt_boxes.append(rcnn_match_anchors_to_gt(anchor_boxes, gt_boxes[i],
                                                             iou_thresholds=self.anchor_iou_thresholds).to(anchor_boxes
                                                                                                           .device))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        if self.training:
            ...
            matched_gt_boxes = matched_gt_boxes.view(-1, 5)  # (B * HWA, 5)
            ...
            # print(num_samples, matched_gt_boxes.shape[0])  # 256 49392
            # assert num_samples == matched_gt_boxes.shape[0]
            fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, num_samples, 0.5)
            idx_chosen = torch.cat((fg_idx, bg_idx))
            anchor_box = anchor_boxes[idx_chosen].to(anchor_boxes.device)
            gt_deltas = rcnn_get_deltas_from_anchors(anchor_box, matched_gt_boxes[idx_chosen])
            # gt_objectness = matched_gt_boxes[idx_chosen, 4]
            gt_objectness = torch.zeros(len(idx_chosen), dtype=anchor_boxes.dtype, device=anchor_boxes.device)
            gt_objectness[:len(fg_idx)] = 1
            # but why? now I know .... the first len(fg_idx) elements in anchor_box is foreground.
            # print(pred_boxreg_deltas.shape, gt_deltas.shape)
            loss_box = F.l1_loss(
                pred_boxreg_deltas[idx_chosen], gt_deltas, reduction="none"
            )
            loss_box[gt_deltas == -1e8] *= 0.0
            loss_obj = F.binary_cross_entropy_with_logits(
                pred_obj_logits.view(-1)[idx_chosen], gt_objectness, reduction="none"
            )
...
#             level_anchors = anchors_per_fpn_level[level_name]  # shape: (HWA, 4)
...
#                 # level_obj_logits = pred_obj_logits[level_name]
#                 # level_boxreg_deltas = pred_boxreg_deltas[level_name]
#                 level_obj_logit = level_obj_logits[_batch_idx]  # (HWA, )
#                 level_boxreg_delta = level_boxreg_deltas[_batch_idx]  # (HWA, 4)
#                 assert level_boxreg_delta.shape[1] == 4
#                 assert level_boxreg_delta.shape[0] == level_obj_logit.shape[0]
#                 proposal_boxes = rcnn_apply_deltas_to_anchors(deltas=level_boxreg_delta, anchors=level_anchors)
#                 # (HWA, 4)
#                 width, height = image_size   # (img_w, img_h)
#                 # each of shape (HWA, 1)
#                 proposal_boxes[:, 0] = torch.clamp(proposal_boxes[:, 0], min=0, max=width)
#                 proposal_boxes[:, 2] = torch.clamp(proposal_boxes[:, 2], min=0, max=width)
#                 proposal_boxes[:, 1] = torch.clamp(proposal_boxes[:, 1], min=0, max=height)
#                 proposal_boxes[:, 3] = torch.clamp(proposal_boxes[:, 3], min=0, max=height)
#                 # print('the shape of level_obj_logit\n if (HWA, ), no need for view', level_obj_logit.shape)  # debug
#                 if level_obj_logit.view(-1).shape[0] >= self.pre_nms_topk:
#                     val, idx = torch.topk(input=level_obj_logit.view(-1), k=self.pre_nms_topk)
#                     topk_proposals = proposal_boxes[idx]
#                 else:
#                     topk_proposals = proposal_boxes[:]
#                     val = level_obj_logit[:]
#                 keep_idx = torchvision.ops.nms(topk_proposals, val, self.nms_thresh)
#                 proposals = topk_proposals[keep_idx]
#                 scores = val[keep_idx].reshape(-1)
#                 if len(scores) < self.post_nms_topk:
#                     level_proposals_per_image.append(proposals)
#                 else:
#                     _, post_nums_idx = torch.topk(scores, self.post_nms_topk)
#                     level_proposals_per_image.append(proposals[post_nums_idx])
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        in_features = roi_size[0] * roi_size[1] * stem_channels[-1]  # 7 * 7 * out_channels
        cls_pred.append(nn.Linear(in_features, self.num_classes + 1))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            # fpn_strides is (4, 8, 16, 32)
            # size of level_feats is (56, 28, 14, 7)
            # sizeof(level_feats) / sizeof(spatial_scale) == box_scale
            roi_feats = torchvision.ops.roi_align(input=level_feats,
                                                  boxes=level_props,
        ...
        for _idx in range(len(gt_boxes)):
            ...
            matched_gt_boxes.append(
                rcnn_match_anchors_to_gt(proposals_per_image, gt_boxes_per_image, self.rpn.anchor_iou_thresholds)
            )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
