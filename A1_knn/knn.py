...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
      dists[i] = torch.sqrt(torch.sum(((x_train[i] - x_test) ** 2).view(num_test, -1), dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # functions from torch.nn or torch.nn.functional.                        #
    #                                                                        #
    # HINT: Try  formulate the Euclidean distance using two broadcast sums #
    #       and a matrix multiply.                                           #
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_reshaped = x_train.view(num_train, -1) # tensor of shape (num_train, C * H * W)
    x_test_reshaped = x_test.view(num_test, -1) # tensor of shape (num_test, C * H * W)
    first_term = torch.sum(x_test_reshaped.pow(2), dim = 1).t() # shape (, num_test)
    mid_term = -2.0 * torch.mm(x_train_reshaped, x_test_reshaped.t())
    last_term = torch.sum(x_train_reshaped.pow(2), dim = 1)
    dists = torch.sqrt(mid_term + first_term.view(1, -1) + last_term.view(-1, 1))
    # ##########################################################################
    # #                           END OF YOUR CODE                             #
    # ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        closet_y, idx = torch.topk(-dists[:, i], k) # tensor of shape (k, ), and the indices
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            if i == num_folds - 1:
                ...
                first = torch.cat(x_train_folds[0 : i])
                second = torch.cat(x_train_folds[i + 1:])
                x_train = torch.cat( (first, second) , 0)
                first = torch.cat(y_train_folds[0 : i])
                second = torch.cat(y_train_folds[i + 1:])
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    max_acc = 0.0 # a very small value
