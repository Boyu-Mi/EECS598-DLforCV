...
def mutate_tensor(
    ...
    for i in range(len(indices)):
        first_index = indices[i][0]
        second_index = indices[i][1]
        value = values[i]
        x[first_index, second_index] = value
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    s = start // 10 * 10 + 10
    e = stop // 10 * 10 + 10
    x = torch.arange(s, e, step = 10, dtype = torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = (x - torch.mean(x, dim=0)) / torch.sqrt(torch.sum((x - torch.mean(x, dim=0)) ** 2, dim=0)/(x.shape[0] - 1))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
