  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
      ...
      if margin > 0:
        ...
        dW[:, j] += (1 / num_train) * X[i] # 这里的损失函数除以了num_train, 梯度也要相应修正！
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  temp = torch.mm(X, W) # a tensor of shape (N, C), row i is the scores of the ith image(X[i]) 128 * 10
  correct_score = temp[range(num_train), y] 
  # a tensor of shape (N, ), correct_score[i] is the score of the correct class of X[i]
  margin = -correct_score.view(-1, 1) + temp + 1
  margin[margin < 0] = 0 # boolean mask, vital
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  num_iters = 100
  # Replace "pass" statement with your code
  losshistory = cls.train(data_dict['X_val'], data_dict['y_val'], lr, reg, num_iters)
  y_train_pred = predict_linear_classifier(cls.W, data_dict['X_train'])
  train_acc = torch.sum(y_train_pred == data_dict['y_train']) / data_dict['y_train'].shape[0] 
