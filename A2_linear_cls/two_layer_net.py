...
def nn_forward_pass(params, X):
    ...
    hidden = X.mm(W1) + b1.view(1, -1)
    hidden[hidden < 0] = 0.0 # ReLU
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    losses = torch.exp(scores[range(N), y]) / torch.sum(torch.exp(scores), dim=1)   # get (N,)
    ...
    # for i in range(N):
    #   y_onehot[i, y[i]] = 1
    # dLdscore = scores - y_onehot # https://zhuanlan.zhihu.com/p/86184547 (N, C) - (N, C)
    #h1 (N, H) 

    grad_matrix = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1).reshape(-1,1)  # (N, C) matrix
    grad_matrix[list(range(N)), y] += -1 #refer to https://zhuanlan.zhihu.com/p/86184547 
    dLdscore = grad_matrix
    dLdb2 = torch.sum(dLdscore, dim=0) / N  # (C, N) * (N, ) = (C, )
    dLdW2 = h1.t() @ dLdscore / N #(H, N) * (N, C) = (H, C)

    dh1 = grad_matrix @ W2.t() #(N, C) * (C, H) = (N, H) 
    
    #FC1 is X * W1 + b1, shape(N, D) * (D, H) + (H, ) = (N, H) 
    #h1 = ReLU(FC1)
    dh1[h1 <= 0] = 0 # (N, H) 

    dLdb1 = torch.sum(dh1, dim=0)
    dLdW1 = X.t() @ dh1  # (D, N) * (N, H) = (D, H)
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  scores, losses = nn_forward_pass(params, X)
...
#             if max(val_acc_history) > best_val_acc:
